...
export default () => {
    ...
    useEffect(() => {
        ...
        axios.get("https://api.themoviedb.org/3/movie/popular?api_key=c49e028232019660cab8e28bf4d018d9&language=en-US&page=27")
            .then(res => {
                res.data.results.map((movie, i) => {
                    if(movie.genre_ids.includes(27)){
                        tempHorrorArray.push(movie)
        ...
        console.log(tempHorrorArray)
        setHorrorMovies(tempHorrorArray)
    }, [])
    ...
    return (
        <div className='size'>
            <Slider ref={(c) => (slider.current = c)} {...settings}>
                {horrorMovies.map(function (movie, i) {
                    return (
                        <div key={i}>
